...
import { useCommunitySongs, useCommunitySong, useFavorites } from '@/hooks/useCommunitySongs';
import { useLiveSession } from '@/hooks/useLiveSession';
import { formatChord, extractChordsFromLine } from '@laudasist/shared';
import type { Key, ChordStyle, Song, SongPart } from '@laudasist/shared';
...
function SessionPageContent() {
    ...
    const { data: communityResults } = useCommunitySongs({ search: searchQuery || undefined });
    const { data: allCommunityData } = useCommunitySongs({});

    // Favorites for guest mode
    ...
    const [chordDisplay, setChordDisplay] = useState<'above' | 'inline' | 'compact'>('above');
    const [recentlyPlayed, setRecentlyPlayed] = useState<string[]>([]);
    const [useOriginalKey, setUseOriginalKey] = useState(true); // Key preference toggle

    // Live broadcasting
    const { isLive, isLoading, startLive, endLive, broadcastUpdate, getShareUrl } = useLiveSession();
    ...
    const goLive = useCallback((song: Song) => {
        ...
        <div className={styles.container}>
            <header className={styles.header}>
                <div className={styles.headerLeft}>
                    <Link href="/dashboard" className={styles.exitLink}>← Exit</Link>
                ...
                <div className={styles.headerRight}>
                    {!isGuest && (isLive ? (
                        <>
                            ...
                            <button onClick={() => { navigator.clipboard.writeText(getShareUrl()); }} className={styles.shareBtn}>
                                📋 Copy Link
                            </button>
                        ...
                        <>
                            <div className={styles.songHeader}>
                                ...
                                <div className={styles.controls}>
                                    ...
                                    <select
                                        value={chordDisplay}
                                        onChange={(e) => setChordDisplay(e.target.value as 'above' | 'inline' | 'compact')}
                                        className={styles.select}
                                    >
...
function LiveLine({ text, displayKey, originalKey, chordStyle, showChords, chordDisplay }: {
    text: string;
    displayKey: Key;
    originalKey: Key;
    chordStyle: ChordStyle;
    ...
    // Format all chords
    const formattedChords = chords.map((c: any) => formatChord(c.chord, displayKey, chordStyle));
    ...
    if (chordDisplay === 'compact') {
        ...
        return (
            <div className={styles.lineCompact}>
                <span className={styles.chordsStart}>{chordPrefix}{formattedChords.join(' ')}</span>
                <span>{cleanText}</span>
            </div>
        );
    }
    ...
    if (chordDisplay === 'inline') {
        ...
        chords.forEach((chordPos: any, i: number) => {
            if (chordPos.index > lastIndex) {
                segments.push(<span key={`t${i}`}>{cleanText.substring(lastIndex, chordPos.index)}</span>);
            }
            segments.push(<span key={`c${i}`} className={styles.chordInline}>[{formattedChords[i]}]</span>);
        ...
        <div className={styles.lineAbove}>
            <div className={styles.chordRow}>
                {chords.map((chordPos: any, i: number) => (
                    <span key={i} className={styles.chordAbove} style={{ left: `${chordPos.index}ch` }}>
                        {formattedChords[i]}
                    </span>
                ))}
            </div>
