...
**Goal**: Provide an in-app chromatic tuner for musicians to tune their instruments before/during worship.

**Approach**:
- Use Web Audio API + `getUserMedia()` to access microphone
- Perform pitch detection using autocorrelation or FFT algorithm
- Display detected note, cents deviation, and visual indicator (sharp/flat/in-tune)
- Support standard tuning (E A D G B E) and alternate tunings
